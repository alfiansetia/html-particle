...
function animate() {
  ...
  // Rotate entire system slowly (Disabled as requested)
...
function onResults(results) {
  ...
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    ...
    // MediaPipe coords are 0 to 1, we map to -1 to 1 for our state
    ...
    // A finger is considered extended if its tip's y-coordinate is less than its PIP joint's y-coordinate (assuming standard hand orientation)
    const isExtended = (tip, pip) => tip.y < pip.y;

    // Check each finger (Simplified detection)
    ...
    // 2. Peace Sign (Index + Middle) -> Star
      ...
      if (state.currentShape !== "star") {
        setShape("star");
        statusText.innerText = "⭐ STAR POWER ⭐";
        material.color.set(0xffd700);
      }
    }
    // 3. Rock On / Spider-man (Index + Pinky) -> Torus
    else if (
      indexExtended &&
      pinkyExtended &&
      !middleExtended &&
      ...
      if (state.currentShape !== "torus") {
        setShape("torus");
        statusText.innerText = "🤘 ROCK ON / TORUS 🤘";
        material.color.set(0x00ffcc);
      }
    }
    // 4. Index Pointing -> Helix
    else if (
      indexExtended &&
      !middleExtended &&
      !ringExtended &&
      ...
      if (state.currentShape !== "helix") {
        setShape("helix");
        statusText.innerText = "🧬 HELIX ENERGY 🧬";
      }
    }
    // 5. Open Palm -> Galaxy or Sphere
    else if (extendedCount >= 4) {
      if (state.currentShape !== "galaxy" && state.currentShape !== "sphere") {
        setShape("galaxy");
        statusText.innerText = "🌌 OPEN GALAXY 🌌";
      }
    }
    // 6. Fist -> Cube
    else if (extendedCount === 0 && loveDist > 0.1) {
      if (state.currentShape !== "cube") {
        setShape("cube");
        statusText.innerText = "📦 CUBE MODE 📦";
      }
    }
...
function setShape(shape) {
  ...
  // UI Active State
  document
    .querySelectorAll(".btn-grid button")
